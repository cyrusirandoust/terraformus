...
locals {
  ...
  password_command     = "$password = ConvertTo-SecureString ${chomp(replace(var.admin_password == null ? element(concat(random_password.passwd.*.result, [""]), 0) : var.admin_password, "$", "$$$`$", "\\", "\\\\"))} -AsPlainText -Force"
